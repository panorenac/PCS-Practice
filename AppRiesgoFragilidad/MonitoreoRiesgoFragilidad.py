...
def main():
  ...
  # Simulación terminada, graficar resultados
    ...
    riesgos = [evento['Riesgo'] for evento in registro_eventos]
    
    # Graficar el ritmo cardiaco a lo largo del tiempo
    plt.plot(tiempos, ritmos, marker='o', label='Ritmo Cardiaco (lpm)', color='blue')
    plt.title("Ritmo Cardiaco y Riesgo de Fragilidad a lo Largo del Tiempo")
    ...
    for tiempo, ritmo, riesgo in zip(tiempos, ritmos, riesgos):
        ...
        plt.text(tiempo, ritmo + 2, riesgo, color=color, fontsize=9, ha='center')  # Etiquetas "Alto" o "Bajo"
